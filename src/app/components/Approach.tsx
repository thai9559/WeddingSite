...
export function Approach() {
  ...
  // 🔹 Load danh sách video (đọc nội dung URL từ file .txt)
  const loadVideos = useCallback(async (type: VideoType) => {
    ...
    try {
      ...
      for (const f of txtFiles) {
        ...
        const { data: fileData } = await supabaseRef.current.storage
          .from(BUCKET)
          .download(path);

        let url = "";
        if (fileData) {
          url = await fileData.text();
        }
        ...
        const id =
          ...
          extractVimeoId(url) ||
          f.name.replace(".txt", "");
        videosList.push({ video_id: id, type, url });
      }
                ...
                {videos.map((video, index) => {
                  ...
                  const thumb =
                    type === "youtube" && extractYouTubeId(video.url)
                      ...
                      : "https://cdn-icons-png.flaticon.com/512/1384/1384060.png"; // Vimeo fallback
